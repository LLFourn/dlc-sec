...
\begin{figure}[h]
  ...
  \end{pchstack}
  \begin{pchstack}[center]
    \procedure{$\Anticipate(\pk, \eventid, \ann, \att, o)$}{
        X := \pk; R := \ann \\
        c \gets H(R, X, \eventid, o) \\
        \pcreturn R + cX
    }
...
However, note that just because valid Schnorr signatures result from the scheme in Figure~\ref{dryja} does not mean that the signatures are unforgeable.
...
In our case here the exact message is unknown when the honest signer chooses $R$ which means the proof does not apply.
...
% \newcommand{\mAttO}{\mathsf{mAtt}}
% \newcommand{\mAnnO}{\mathsf{mAnn}}

% \begin{proof}
%   We use a forger $\F$ against the multi-party unforgeability of Dryja's scheme to construct an adversary $\adv$ against $\OMDL$.
%   Just as in the single oracle case we take the first challenge group element $X_{0}$ and use it as our oracle's public key and then use the remaining elements $X_{1}, \ldots, X_{n}$ as the announcements when simulating the $\mAnnO$ oracle and finally use our $\ODL$ oracle to simulate the $\mAttO$ oracle to the forger.
%   Once $\F$ returns a forged attestation under a previous announcement we use the nonce secret keys $r^{*}_{1},\ldots,r^{*}_{n}$ that have access to in the KOSK model to extract $x_{0}$.
%   With $x_{0}$ in hand we can work backwards just as in the single oracle case to obtain $x_{1},\ldots,x_{n}$ and therefore output the $\OMDL$ solution.
...
%           (\eventid^*, \ann^*, \att,c, o) := Q_{i^*} \\
%           \pccomment{We know $r_{i}$ in KOSK model} \\
%           ((R^*_1,r^*_1),\ldots, (R^*_n, r^*_n)) := \ann^* \\
%           c^* \gets H(X_{i^*}, X_0, \eventid^*, o^*) \\
%           \pcif c = c^* \pcthen \pcabort \\
%           x_0 \gets (\att - \att^* - \Sigma{i}^n r_i )/(c - c^*) \\
%           \pcfor j = 1, \ldots, n \pcdo \\
%           \t (\eventid_j, \att_j,c_j, o_j) := Q_j \\
%           \t x_j \gets \att_j - c_jx_0 \\
%           \pcreturn (x_0,\ldots, x_n)
...
%       \pchspace
%       \begin{pcvstack}
%         \procedure{ Simulate $\mAnnO(\eventid_{i})$ }{
%           \ann_i := X_i \\
%           i \gets i + 1 \\
%           \pcreturn \ann_i
%         }
%         \pcvspace
%         \procedure{ Simulate $\mAttO(\eventid_{i}, o_i, \ann_{i})$ }{
%           c_i \gets H(X_i, X_0, \eventid_i, o_i) \\
%           S_i \gets X_i + cX_0 \\
%           \att_i \gets \ODL(S_i) \\
%           Q_i := (\eventid_i, \att_i,c_i, o_i) \\
